...
async function loadData(){
  ...
  // ── 8개 fetch 병렬 실행 (characters는 위 체인 재사용) ────────
  const [noticesRes,loginRes,erasRes,talentsRes,soopRes,dimRes,charsRes,gsLogRes,dungeonSrcRes]=await Promise.all([
    fetch('/data/notices.json').catch(()=>null),
    fetch(_loginUrl).catch(()=>null),
    fetch('/data/itemsEra.json').catch(()=>null),
    ...
    _charsProm,
    (_isLocal?fetch(`/gearscore_log.json?v=${Date.now()}`):fetch(`${_STORAGE}/gearscore_log%2Fgearscore_log.json?alt=media&v=${Date.now()}`)).catch(()=>null),
    fetch('/data/tbc_dungeon_sources.json').catch(()=>null),
  ]);

  // ── JSON 파싱 병렬 실행 ──────────────────────────────────────
  const [noticesData,loginRows,itemsEraRaw,tbcRaw,soopArr,apArr,allDocs,gsLogArr,dungeonSrcRaw]=await Promise.all([
    noticesRes?.ok?noticesRes.json().catch(()=>[]):Promise.resolve([]),
    loginRes?.ok?loginRes.json().catch(()=>[]):Promise.resolve([]),
    erasRes?.ok?erasRes.json().catch(()=>({})):Promise.resolve({}),
