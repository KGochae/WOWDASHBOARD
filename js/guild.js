...
function applyGuildFilter(){
  ...
  listEl.innerHTML=names.map(n=>{
    ...
    return `<div class="guild-row${n===_ac2?' active':''}"
      ...
      onclick="quickSelect('${n.replace(/'/g,"\\'")}')">
      ${avatarHtml}
      <div class="gr-info">
...
function renderLoginHeatmap(name, rankName){
  ...
  // 범위: 전달 1일 ~ 현재 월 말일 (2개월 연속 GitHub 스타일)
  const now=new Date();
  ...
  const startDow=(rangeStart.getDay()+6)%7; // Mon=0
  const gridStart=new Date(prevYear,prevMonth,1-startDow);
...
function selectChar(name){
  ...
  const lastLogin=(()=>{
    ...
    if(!kst)return '';
    // "2026-04-07 01:28:48" -> "26.04.07 01:28:48"
    const [date,time]=kst.split(' ');
    const [yy,mo,dd]=date.split('-');
    return `${String(yy).slice(2)}.${mo}.${dd} ${time}`;
  })();
  // 스킬 pts 표시
  ...
  document.getElementById('charBadges').innerHTML=`
    ...
    ${soopEntry?.soop_url
      ?`<a class="cbadge cbadge-soop" href="${soopEntry.soop_url}" target="_blank" rel="noopener"><img src="/images/logo/SOOP_LOGO_White.svg" class="cbadge-soop-logo" alt="SOOP"></a>`
      :(gm.rank_name?`<span class="cbadge cbadge-rank">${gm.rank_name}</span>`:'')}
